...
class WebD3Chart extends React.Component {
  ...
  drawChartBarHor = () => {
    ...
    const padding = { left: 40, top: 10, right: 20, bottom: 20 };
    ...
    let xScale = d3.scaleLinear().domain([10, d3.max(values) * 1.2]).range([40, (chartWidth * .9)]);
    // let yScale = d3.scaleLinear().domain([10, d3.max(values) * 1.2]).range([40, maxHeight]);
    ...
    const resize = window.onresize = () => {
      ...
      xScale = d3.scaleLinear().domain([10, d3.max(values) * 1.2]).range([40, (chartWidth * .9)]);
    ...
    const updateElements = () => {
      ...
      totalLabels.data(dataValue, (d, i) => d.name).text(d => scientificToNumber(d.value)).transition().duration(animationDurationTime)
        .attr("x", d => xScale(d.value) + rect.marginV + font.margin + 30)
        .attr("y", (_, i) => (rect.marginV + rect.height) * i + rect.marginT + rect.height / 2);
