...
import {
  ...
  Modal,
  Tooltip,
  ConfigProvider, Layout, Menu, Icon, Typography, Divider,
} from 'antd';
...
const setup = (ctx) => {
  ...
  const mutiPoint = times => new Array(times).fill('.')
    .join('');
...
const CtrlBox = React.memo((props) => {
  ...
  const getMp4Url = filePath => `http://${window.location.host}/dist/${filePath}/out.mp4`;
